...
### AI Integration: Grok-2 (via xAI)
I decided to use xAI's Grok-2 model instead of OpenAI's GPT-4 or Google's Gemini. The main reason is that it's insanely fast and has a huge context window, which is perfect for dumping long meeting transcripts into the prompt without having to do complex vector embeddings or RAG setups.
